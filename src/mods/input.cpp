...
uint16_t mapShortcutFromText(const char *text) {
    uint16_t result = 0;
    while (text) {
        const auto *tok = strchr(text, '+');
        ...
        char name[64];
        memcpy(name, word, std::min((size_t)(tok - word), (size_t)63));
        name[tok - word] = 0;
        if (strcasecmp(name, "SHIFT") == 0) {
            ...
            result |= 0x2000u;
        } else if (!(result & 0x1FFFu)) {
            result |= mapKeyFromName(name);
        }
    }
    return result;
}
